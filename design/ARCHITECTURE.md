...
- **`renderSpec`**: symbols never build DOM. `Symb.renderSpec(game, x, y)`
  ...
  `🧑‍🎄`, `displayEmoji()` returns a random skin-tone `🎅` variant drawn by
  `rollCosmetics()` and fixed until the next catalog build (see
  `Catalog`'s cosmetic-reroll hook, `catalog.js`).
...
- Santa's logical identity (`emoji()`) is the deterministic `🧑‍🎄`; only
  its cosmetic `displayEmoji()` skin-tone variant is chosen by
  `Catalog`'s cosmetic-reroll hook (`rollCosmetics()`, see `catalog.js`),
  and that value never reaches the trace (see "Display glyph vs. logical
  identity" below), so no normalization is needed.
